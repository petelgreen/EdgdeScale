...
- **lib/** — shared code: `config.py` (env vars), `consts.py` (queue names, timeouts, limits), `telemetry_client.py` (log sender).
...
**Word count accuracy:** Service A publishes raw gRPC chunks as-is — no boundary logic. Service B owns `split_at_word_boundary` and carries a `leftover` string in `_file_state[cid]` between chunks. On each chunk it prepends the leftover from the previous chunk, splits at the last word boundary, counts the complete words, and stores the partial word for the next chunk. The EOS marker (empty body, `is_last=1`) triggers a final flush of whatever leftover remains, then publishes the total word count. Words that span a raw gRPC chunk boundary are never double-counted.
...
│       ├── file.py            # file upload — streams raw chunks, sends EOS
